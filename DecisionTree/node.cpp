...
int Node::getEstimatedValue(const std::vector<Chess*>& aiCandidate, const std::vector<Chess*>& playerCandidate)
{
	...
	constexpr static int chessValue[8] = { 0,400,1500,2500,1500,800,800,/*100000000*/ 0 };
	constexpr static int moveValue[8] = { 0,0,1,5,10,1,1,0 };
	...
	int totalValue = 0;
	int confilictMap[10][9]{}; 
	int threatenMap[10][9]{};
	...
	for (auto& chess : aiCandidate)
	{
		...
		if (type != None)
		{
			...
			totalValue += moveValue[type] * target->moveableList.length;
		}
	}
	...
	for (auto&chess : aiCandidate)
	{
		if (chess->type != None)
		{
			auto x = chess->getX();
			auto y = chess->getY();
			if (confilictMap[y][x] > 0)
			{
				totalValue -= chessValue[chess->type] / 4;
			}
